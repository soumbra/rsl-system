      ...
      <v-col cols="12" sm="8" md="6" lg="4">
        <v-card elevation="8" class="pa-6">
          ...
          <v-card-text>
            <!-- Status da conexão -->
            <v-alert
              v-if="backendStatus.isConnected"
              type="success"
              class="mb-4"
              variant="tonal"
            >
              ...
              <div class="text-caption mt-2">
                <div><strong>Serviço:</strong> {{ backendStatus.health?.service }}</div>
                <div><strong>Versão:</strong> {{ backendStatus.health?.version }}</div>
                <div><strong>Nome:</strong> {{ backendStatus.info?.name }}</div>
                <div><strong>Java:</strong> {{ backendStatus.info?.java_version }}</div>
                <div><strong>Spring:</strong> {{ backendStatus.info?.spring_boot }}</div>
                <div><strong>CORS:</strong> {{ backendStatus.corsTest?.frontend_can_access ? 'OK' : 'ERRO' }}</div>
              </div>
            </v-alert>
              ...
              <v-expansion-panel>
                ...
                <v-expansion-panel-text>
                  <div class="text-caption">
                    <strong>Health Service:</strong> {{ backendStatus.health?.service }}<br>
                    <strong>Health Version:</strong> {{ backendStatus.health?.version }}<br>
                    <strong>Info Name:</strong> {{ backendStatus.info.name }}<br>
                    <strong>Descrição:</strong> {{ backendStatus.info.description }}<br>
                    <strong>Java:</strong> {{ backendStatus.info.java_version }}<br>
                    <strong>Spring Boot:</strong> {{ backendStatus.info.spring_boot }}<br>
                    <strong>URL:</strong> http://localhost:8080/api<br>
                    <strong>CORS Test:</strong> {{ backendStatus.corsTest?.message }}<br>
                    <strong>Frontend Access:</strong> {{ backendStatus.corsTest?.frontend_can_access ? 'Sim' : 'Não' }}
                  </div>
                </v-expansion-panel-text>
              </v-expansion-panel>
            </v-expansion-panels>
